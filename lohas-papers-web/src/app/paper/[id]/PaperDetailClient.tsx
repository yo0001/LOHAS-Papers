...
interface Props {
  paperId: string;
  initialDetail: PaperDetailResponse | null;
}

export default function PaperDetailClient({ paperId, initialDetail }: Props) {
  const { locale } = useLanguage();
  const { refreshCredits } = useAuth();
  const [showLogin, setShowLogin] = useState(false);

  const [detail, setDetail] = useState<PaperDetailResponse | null>(
    initialDetail,
  );
  ...
  // Fulltext
  const [fulltextSections, setFulltextSections] = useState<
    FulltextSection[] | null
  >(null);
  const [fulltextLoading, setFulltextLoading] = useState(false);
  const [fulltextDifficulty, setFulltextDifficulty] =
    useState<Difficulty>("layperson");
  const [fulltextError, setFulltextError] = useState<string | null>(null);

  // Vocabulary
  const [vocabularyData, setVocabularyData] =
    useState<VocabularyAnalysisResponse | null>(null);
    ...
    try {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          console.error(
            `Paper detail fetch error (attempt ${attempt + 1}/3):`,
            e,
          );
          lastError = e;
          if (attempt < 2)
  ...
  const loadFulltext = async () => {
    ...
    try {
      const res = await getFulltextTranslationWithAI(
        paperId,
        locale,
        fulltextDifficulty,
      );
  ...
  const loadVocabulary = async () => {
    ...
    try {
      ...
      if (e instanceof ServiceUnavailableError) {
        setVocabularyError(
          "AIサービスが一時的に利用できません。しばらく待ってからお試しください。",
        );
        setVocabularyLoading(false);
        return;
      }
  ...
  return (
    <>
      ...
      <div className="space-y-2 text-sm text-gray-600">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {detail.citation_count > 0 && (
            <span>
              {t(locale, "citations")}: {detail.citation_count}
            </span>
          )}
          ...
          <div className="flex items-center justify-between">
            ...
            >
              {showOriginalAbstract
                ? t(locale, "abstractTranslated")
                : t(locale, "abstractOriginal")}
            </button>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
        <h2 className="font-semibold text-gray-900">
          {t(locale, "fulltextTranslation")}
        </h2>

        {!detail.is_open_access ? (
          <p className="text-sm text-gray-500">
            {t(locale, "fulltextNotAvailable")}
          </p>
        ) : fulltextSections ? (
          ...
          <div className="space-y-3">
            <DifficultyTabs
              selected={fulltextDifficulty}
              onChange={setFulltextDifficulty}
            />
            <button
